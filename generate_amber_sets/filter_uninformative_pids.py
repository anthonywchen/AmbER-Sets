...
def filter_pids(input_data_file, good_pids_file):
    ...
    for d in tqdm(jsonlines.open(input_data_file)):
        for qid, qid_dict in d["qids"].items():
            qid_types = set(
                [type for type in qid_dict["entity_types"] if type in good_pids_dict]
            )

            # Filter inter PIDS
            inter_pids = list(
                itertools.chain(*[good_pids_dict[type]["inter"] for type in qid_types])
            )
            inter_pids = [x for x in inter_pids if inter_pids.count(x) == 1]

            intra_pids = list(
                itertools.chain(*[good_pids_dict[type]["intra"] for type in qid_types])
            )
...
def statistics(alias_dicts, output_stats_file):
    ...
    for d in alias_dicts:
        num_qids = len(
            [1 for qid_dict in d["qids"].values() if len(qid_dict["pids"]) > 0]
        )
        counts[num_qids] += 1
