...
def filter_relations(collection: str) -> None:
    ...
    entity_types_to_distinguishing_properties = json.load(
        open(entity_types_to_distinguishing_properties_file)
    )
    ...
    for d in tqdm.tqdm(polysemous_names, desc="Filtering relations"):
        ...
        for qid in d["qids"]:
            ...
            distinguishing_properties = list(
                itertools.chain(
                    *[
            ...
            for pid in list(d["qids"][qid]["pids"].keys()):
                if (
                    pid not in distinguishing_properties
                    or pid_counts[pid] > 1
                    or len(d["qids"][qid]["pids"][pid]["values"]) == 0
                ):
