        ...
        for pid in qid_to_pids[qid]:
            ...
            for answer_dict in qid_to_pids[qid][pid]:
                if answer_dict["type"] == "entityid":
                    answer_dict["entity_types"] = [
                        e["qid"]
                        for e in qid_to_pids.get(answer_dict["qid"], {}).get("P31", [])
                    ]
                    ...
                    if answer_qid in qid_to_pids:
                        for instance in qid_to_pids[answer_qid].get("P3095", []):
                            ...
                            answer_dict["additional_aliases"].update(
                                qid_to_aliases.get(participant_qid, [])
                            )
...
def load_qid_to_pids(
    qid_to_pids_file, qid_to_aliases_file, pid_to_label_file, good_pids_file
):
...
def merge_aliases_and_pids(alias_to_qids_file, qid_to_pids):
    ...
    alias_dicts = []
    for alias, qids in tqdm(
        json.load(open(alias_to_qids_file, encoding="utf-8")).items()
    ):
        alias_dict = {
            "name": alias,
            "qids": {
                e["qid"]: {
                    ...
                    "is_head": e["is_head"],
                    "entity_types": [
                        e["qid"] for e in qid_to_pids[e["qid"]]["P31"]["values"]
                    ],
                }
                for e in qids
                if e["qid"] in qid_to_pids
            },
        }
